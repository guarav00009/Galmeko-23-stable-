...
def hospital_soft_delete(request): 
    result = {}
    if request.method == 'POST' and request.is_ajax(): 
        try:
            Hospital_userId = request.POST.get('id', '')
            response = Hospital.objects.filter(pk=Hospital_userId).update(status=3)
            if (response == True):
                result['status'] = True
            ...
            return HttpResponseRedirect("/admin/hospital/hospital/")
    else:
        return HttpResponse('Invalid request passed')
